...
class HostStorageBootstrap:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(initialization_allowed, bool):
            raise DevelopmentEnvironmentError("Retained EBS initialization authorization is invalid")
        ...
        if not device_by_id_root_path.is_absolute() or not fstab_path.is_absolute():
            raise DevelopmentEnvironmentError("Retained storage system paths must be absolute")
    ...
    def mount(self) -> None:
        ...
        device_path = Path(
            ...
            / ("nvme-Amazon_Elastic_Block_Store_" + self._retained_volume_id.replace("-", ""))
        )
        ...
        if not device_path.exists():
            raise DevelopmentEnvironmentError("Retained EBS device did not appear within the bounded wait")
        ...
        if filesystem_type_result.returncode != 0:
            if not self._initialization_allowed:
                raise DevelopmentEnvironmentError(
                    "Retained EBS device has no recognized filesystem and " "initialization is not authorized"
                )
            self._runner.run(["mkfs.xfs", str(device_path)])
            filesystem_type_result = self._runner.run(["blkid", "-s", "TYPE", "-o", "value", str(device_path)])
        if filesystem_type_result.stdout.strip() != "xfs":
            raise DevelopmentEnvironmentError("Retained EBS device does not contain the required XFS filesystem")
        uuid_result = self._runner.run(["blkid", "-s", "UUID", "-o", "value", str(device_path)])
        ...
        fstab_line = (
            f"UUID={volume_uuid} {self._retained_root_path} xfs defaults,nofail,x-systemd.device-timeout=30 0 2"
        )
        ...
        if target_entry_list not in ([], [fstab_line]):
            raise DevelopmentEnvironmentError("Retained root has another persistent mount declaration")
        ...
        if mounted.returncode == 0:
            if mounted.stdout.strip() != volume_uuid:
                raise DevelopmentEnvironmentError("Retained root is mounted from another filesystem")
        else:
