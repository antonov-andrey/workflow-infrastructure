...
HOST_ARTIFACT_MANIFEST_PATH = Path("/etc/workflow-infrastructure/host-artifact-manifest.json")
HOST_ARTIFACT_MANIFEST_SHA256_PATH = Path("/etc/workflow-infrastructure/host-artifact-manifest.sha256")
...
class ProductToolProtocol(Protocol):
    """Current Product tool boundary consumed by host lifecycle."""
...
class DevelopmentHostManager:
    ...
    def __init__(
        ...
        *,
        ...
        is_host: bool,
        product_recovery_state: ProductRecoveryStateProtocol,
        ...
        self._product_recovery_state = product_recovery_state
    ...
    def host_product_activity_get(self) -> str:
        ...
        try:
            ...
            return "busy"
        product_tool_path = self._product_tool.path_get()
        if not product_tool_path.is_file():
            return "busy"
        result = self._runner.run(
            self._product_tool.command_list_get("activity"),
            check=False,
        )
...
"""
    ...
    def host_shutdown(self) -> None:
        ...
        if product_tool_path.is_file():
            result = self._runner.run(
                self._product_tool.command_list_get("shutdown"),
                check=False,
