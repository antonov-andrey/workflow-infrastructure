    ...
    for line in gpg_colon_output.splitlines():
        ...
        if record_type == "pub":
            if primary_fingerprint_is_pending:
                raise HostArtifactResolutionError("signing keyring has a primary key without a fingerprint")
            ...
            if len(field_list) <= 9 or re.fullmatch(r"[0-9A-F]{40}", field_list[9]) is None:
                raise HostArtifactResolutionError("signing keyring has a malformed primary fingerprint")
...
def checksum_file_sha256_get(*, artifact_name: str, checksum_path: Path) -> str:
    ...
    for line in line_list:
        match = re.fullmatch(r"([0-9a-f]{64})[ \t]+(?:\*|)" + re.escape(artifact_name), line)
...
class HostArtifactVerifier:
    ...
    def signing_key_validate(self, *, key_path: Path, expected_primary_fingerprint: str) -> None:
        ...
        if primary_key_fingerprint_list_get(result.stdout) != [expected_primary_fingerprint]:
            raise HostArtifactResolutionError("signing keyring does not contain exactly the trusted primary key")
    ...
    def pgp_detached_signature_validate(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            if primary_key_fingerprint_list_get(key_payload.stdout) != [expected_primary_fingerprint]:
