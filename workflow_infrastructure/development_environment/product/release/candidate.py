...
def _descriptor_validate(value: object, *, expected_digest: str) -> None:
    ...
    if (
        ...
        raise DevelopmentEnvironmentError("Retained Product OCI graph descriptor is invalid")
...
def _archive_validate(
    *,
    ...
    if (
        ...
        raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} archive identity is invalid")
    ...
    try:
        ...
        raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} OCI archive is unsafe") from error
    if digest.hexdigest() != archive_sha256:
        raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} OCI archive differs")
    ...
    def validate(
        ...
        *,
        ...
        if hashlib.sha256(ledger_bytes).hexdigest() != expected_ledger_sha256:
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} candidate ledger differs")
        if not isinstance(ledger, Mapping) or set(ledger) != {
            ...
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} candidate ledger is malformed")
        expected_temporary_tag = (
            f"candidate-{release_root_path.name}-" f"{hashlib.sha256(logical_name.encode()).hexdigest()[:20]}"
        )
        if (
            ledger.get("candidate_identity") != f"ProductRelease:{release_root_path.name}:{logical_name}"
            ...
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} candidate identity is invalid")
        graph = ledger.get("graph")
        archive = ledger.get("archive")
        if not isinstance(graph, Mapping) or not isinstance(archive, Mapping):
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} candidate graph is missing")
        ...
        if (
            ledger.get("completed_manifest_digest_list") != graph.get("publication_manifest_digest_list")
            ...
            or set(ledger.get("completed_manifest_digest_list", [])) != manifest_digest_set
        ):
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} publication proof is incomplete")
        _archive_validate(
            archive=archive,
    ...
    def _graph_validate(
        ...
        *,
        ...
        if set(graph) != {
            ...
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} graph shape is invalid")
        ...
        if (
            ...
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} graph identity is invalid")
        ...
        if (
            ...
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} graph closure is invalid")
        for digest, descriptor in blob_map.items():
            _descriptor_validate(descriptor, expected_digest=digest)
        ...
        for digest, node in manifest_map.items():
            if not isinstance(node, Mapping) or set(node) != {
                ...
                raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} manifest node is invalid")
            _descriptor_validate(node.get("descriptor"), expected_digest=digest)
            ...
            if (
                ...
                raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} manifest edges are invalid")
        ...
        if any(
            ...
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} publication order is invalid")
        ...
        if observed != manifest_digest_set:
            raise DevelopmentEnvironmentError(f"Retained Product image {logical_name} graph has unreachable manifests")
