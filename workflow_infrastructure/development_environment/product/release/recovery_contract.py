...
class RetainedProductReleaseValidator:
    ...
    def _retained_product_image_graph_validate(
        *,
        ...
        if not isinstance(image_payload_by_name_map, Mapping) or set(
            image_payload_by_name_map
        ) != (expected_image_name_set):
            raise DevelopmentEnvironmentError(
                "Retained Product release image graph is incomplete"
            )
        contract_source_identity = source_identity_by_name_map.get(
            "workflow-container-contract"
        )
        for logical_name, image_payload in image_payload_by_name_map.items():
            if not isinstance(logical_name, str) or not isinstance(
                ...
                raise DevelopmentEnvironmentError(
                    "Retained Product release image entry is malformed"
                )
            expected_field_name_set = {
            ...
            if (
                ...
                or image_payload.get("pull_reference")
                != f"localhost:30500/{logical_name}@{digest}"
                ...
                or build_metadata_relative_path
                != f"image-build-metadata/{logical_name}.json"
                or not isinstance(candidate_ledger_sha256, str)
                or re.fullmatch(r"[0-9a-f]{64}", candidate_ledger_sha256) is None
                or candidate_ledger_relative_path
                != f"image-candidate/{logical_name}/candidate-ledger.json"
            ):
                raise DevelopmentEnvironmentError(
            ...
            if (
                not isinstance(base_image_by_name_map, Mapping)
                or not base_image_by_name_map
            ):
                raise DevelopmentEnvironmentError(
                    f"Retained Product image {logical_name} base graph is malformed"
                )
            for base_name, base_identity in base_image_by_name_map.items():
                if (
                    ...
                    or re.fullmatch(r"sha256:[0-9a-f]{64}", base_identity["digest"])
                    is None
                    or not isinstance(base_identity.get("pull_reference"), str)
                    or not base_identity["pull_reference"].endswith(
                        f"@{base_identity['digest']}"
                    )
                    or not isinstance(base_identity.get("selector"), str)
                    or not base_identity["selector"]
                ):
                    raise DevelopmentEnvironmentError(
                        f"Retained Product image {logical_name} base identity is invalid"
                    )
            ...
            if (
                ...
                or not isinstance(
                    metadata_payload.get("buildx.build.provenance"), Mapping
                )
            ):
                raise DevelopmentEnvironmentError(
                    f"Retained Product image {logical_name} build metadata differs"
                )
            RetainedProductCandidateLedgerValidator().validate(
