...
`connect` держит одну SSM port-forwarding session от remote ingress. Primary использует persisted port `8080`; task environment получает deterministic collision-checked persisted port, который `connect` и `status` печатают как exact endpoint. Инфраструктура передаёт этот exact port во все WCC Product lifecycle commands; task deployment без него отклоняется. UI build, backend issuer/Host, ZITADEL callbacks/issuer, GlitchTip public URL, ingress hostPort, host-side readiness и ingress CORS используют соответствующие `*.localhost:<environment-port>` origins. `connect` направляет local environment port в тот же remote environment port; отдельной внутренней HTTP identity нет. Команда является long-running foreground process, чтобы состояние tunnel оставалось видимым.
