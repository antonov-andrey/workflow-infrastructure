...
Не запускайте эту команду вручную: она отвергает invocation без exact closed JSON stdin `{schema_version, common_prefix}`, а registered handler владеет natural Project/issue/repository identity, lifetime, fixed invocation и readback. `destroy-inventory` использует тот же request для fresh non-mutating exact `retained|absent` provider readback. Перед каждым cleanup phase и final proof resolver заново объединяет live stack outputs с deterministic bucket names, service-native exact-tag-filtered EC2/EBS reads и complete regional KMS key enumeration; каждый EC2/KMS candidate непосредственно перечитывается и повторно доказывает exact type, task owner, development account и region до mutation. Private journal хранит только progress phase, не resource IDs. `destroy` закрывает sessions/leases, удаляет все task instances и compute stack, затем data stack, чтобы отозвать её IAM writers до очистки exact versioned buckets и multipart uploads. После этого он удаляет все retained task volumes и one-time snapshots, затем KMS alias, disables все task keys и принимает `PendingDeletion` с minimum AWS waiting period. Отдельная final live enumeration обязана доказать отсутствие; поздно появившийся task resource блокирует success и удаляется owning phase следующего resume. Exact machine-readable result содержит common prefix и `external_resources_absent`; foreign resources, account-foundation, primary и другие task environments сохраняются.
