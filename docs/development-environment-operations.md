...
Не запускайте эту команду вручную: она отвергает invocation без exact closed JSON stdin `{schema_version, common_prefix}`, а registered handler владеет natural Project/issue/repository identity, lifetime, fixed invocation и readback. `destroy-inventory` использует тот же request для fresh non-mutating exact `retained|absent` provider readback. Перед каждым cleanup owner и final proof resolver заново объединяет live stack outputs с deterministic bucket names, service-native exact-tag-filtered EC2/EBS reads и complete regional KMS key enumeration; каждый EC2/KMS candidate непосредственно перечитывается и повторно доказывает exact type, task owner, development account и region до mutation. Cleanup не читает и не создаёт progress journal: каждый restart повторяет idempotent owners из current live state, поэтому stale/partial progress file не блокирует и не разрешает действие. `destroy` закрывает sessions/leases, удаляет все task instances и compute stack, затем data stack, чтобы отозвать её IAM writers до очистки exact versioned buckets и multipart uploads. Run-local set сохраняет exact task-owned instance IDs, замеченные fresh enumeration в этой invocation; targeted SSM `Active` reads продолжаются для всех этих IDs после исчезновения EC2 instance, connected/disconnected sessions явно завершаются, а final fresh read обязан доказать их отсутствие. Set не сохраняется и на restart заново выводится из live EC2/SSM state. После этого retained volume в `in-use` ждёт `available`, `available` удаляется, `deleting` ждёт только `deleted`, а `deleted`/exact `NotFound` принимаются как absence; затем удаляются one-time snapshots, KMS alias, disables все task keys и принимается `PendingDeletion` с minimum AWS waiting period. Отдельная final live enumeration обязана доказать отсутствие; поздно появившийся task resource блокирует success и удаляется owning phase следующего restart. Exact machine-readable result содержит common prefix и `external_resources_absent`; foreign resources, account-foundation, primary и другие task environments сохраняются.
