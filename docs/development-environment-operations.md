...
`connect` держит одну SSM port-forwarding session от remote ingress. Primary использует persisted port `8080`; task environment получает deterministic port, который data-plane stack сохраняет как `LocalHttpPort` и атомарно резервирует account-global SSM Parameter `/development/http-port/<port>`. Collision с уже существующей environment поэтому отклоняется самим AWS даже при конкурентном provisioning. `connect` и `status` печатают exact endpoint. Инфраструктура передаёт этот exact port во все WCC Product lifecycle commands; task deployment без него отклоняется. UI build, backend issuer/Host, ZITADEL callbacks/issuer, GlitchTip public URL, ingress hostPort, host-side readiness и ingress CORS используют соответствующие `*.localhost:<environment-port>` origins. `connect` направляет local environment port в тот же remote environment port; отдельной внутренней HTTP identity нет. Команда является long-running foreground process, чтобы состояние tunnel оставалось видимым.
...
Idle controller проверяет только наличие Session Manager session для exact instance. Содержимое port-forwarding и SSH-over-SSM не записывается и для idle-решения не требуется; Session Manager не поддерживает content logging этих tunnel sessions. Ordinary Session Manager shell по account-foundation preferences пишет command/output в KMS-encrypted CloudWatch log group с retention, но не становится вторым источником activity truth. Отдельное Session Manager KMS-шифрование интерактивного channel не включено и не требуется для шифрования CloudWatch logs.
...
Не запускайте эту команду вручную: она отвергает invocation без exact closed JSON stdin `{schema_version, common_prefix}`, а registered handler владеет natural Project/issue/repository identity, lifetime, fixed invocation и readback. `destroy-inventory` использует тот же request для fresh non-mutating exact `retained|absent` provider readback. Перед каждым cleanup owner и final proof resolver заново объединяет live stack outputs с deterministic bucket names, service-native exact-tag-filtered EC2/EBS reads и complete regional KMS key enumeration; каждый candidate непосредственно перечитывается и повторно доказывает exact type, task owner, development account и region до mutation. Cleanup не читает и не создаёт progress journal: каждый restart повторяет idempotent owners из current live state, поэтому stale/partial progress file не блокирует и не разрешает действие. `destroy` останавливает discoverable running task instances, завершает их connected/disconnected SSM sessions и доказывает fresh targeted session absence до compute deletion. Terminated EC2 tombstone остаётся в exact-tag inventory, пока AWS его показывает; если EC2 уже отвечает `NotFound`, exact current compute-stack output сохраняет target до SSM proof и stack deletion, поэтому recovery восстанавливает identity из live AWS owners. Stack заново доказывает name, parameters, tags, account и region и удаляется/ожидается только по unique `StackId` ARN; same-name foreign replacement не затрагивается. Затем data stack удаляется до retained buckets, чтобы отозвать IAM writers; каждый S3 abort/delete повторно доказывает deterministic name, expected owner, current region и exact task tags непосредственно перед mutation. Retained volume в `in-use` ждёт `available`, `available` удаляется, `deleting` ждёт только `deleted`, а `deleted`/exact `NotFound` принимаются как absence; затем удаляются one-time snapshots, KMS alias, disables все task keys и принимается `PendingDeletion` с minimum AWS waiting period. Fresh final enumeration запускает ещё один полный owner pass в той же invocation, если появился новый exact task resource; success требует отдельного service-native absence readback. Exact machine-readable result содержит common prefix и `external_resources_absent`; foreign resources, account-foundation, primary и другие task environments сохраняются.
