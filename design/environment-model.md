...
Каждая инсталляция имеет immutable environment class `development` или `production` и stable environment name. Environment name входит в account-local resource names, release identity, retained roots и operator selection, но не меняет Product domain identity. В dedicated account AWS name содержит только resource role и environment identity; project/repository prefix в stack, role, bucket, alias, log group, scheduler group или EC2 `Name` запрещён. Globally unique names добавляют account ID и region. Kubernetes/Product/repository identities остаются domain names и не считаются AWS account prefix.

Development environment с name `primary` является default. Каждый active task common prefix из canonical `project-goals` при инфраструктурной проверке получает одну полную изолированную development environment, общую для всех participating repository worktrees этой задачи: отдельные data-plane stacks/resources, compute resources, retained state, registry, release pointer, credentials, tunnel и lifecycle controller. Несколько задач не разделяют один mutable cluster, Product database, Data/Athena state или release root скрыто.

Task environment name равен `w` плюс первые 15 lowercase hexadecimal symbols SHA-256 exact task common prefix. До provisioning full prefix сверяется с persisted environment identity; collision одного short name с другим prefix закрывает operation. Каждый taggable task-specific AWS resource имеет exact correlation tag `git-worktree=<common-prefix>`. `primary` и account-foundation этого tag не имеют. Tag является leak check и correlation, но не единственным deletion selector.

Account-global guards, которые физически не могут существовать отдельно для каждой environment, не становятся environment-local data plane. Один account-foundation stack владеет account-level S3 Block Public Access, Lake Formation default settings, account-level Session Manager preferences and encrypted shell log group, а в development также единственным primary-only AWS Backup plan/selection. Environment stacks никогда не создают competing guard owner; до и после apply они fail closed проверяют exact foundation outputs and live state. Environment platform roles получают явные catalog, data-location, database и table permissions только на собственные resources; broad administrator exception development account не переносится в production.
...
OCI registry является adapter. Development может использовать retained cluster-local registry; production использует managed registry. Runtime identity всегда является immutable digest, независимо от реализации registry. Product build сначала создаёт retained local OCI archive без registry mutation; до первого registry PUT release ledger durable фиксирует archive identity, exact candidate repository/tag/platform/owner, root digest и complete root/runnable/attestation manifest graph. Trusted publication загружает recorded graph root-last. Retention обязан удалить весь недостижимый manifest graph по digest до release-directory removal и blob GC даже если final release manifest не был создан.
...
| --- | --- | --- |
| Compute | По одному `m7g.xlarge`, single-node k3s на active development environment | Amazon EKS, multi-AZ managed node groups |
...
Semantic acceptance отдельно проверяет, что development и production docs не дублируют Product implementation, не выдают development exceptions за production requirements, используют account-foundation как единственного owner global guards и не возвращают project prefixes в dedicated-account AWS identities.
