...
Account-global guards, которые физически не могут существовать отдельно для каждой environment, не становятся environment-local data plane. Account-level S3 Block Public Access и Lake Formation default settings имеют одинаковое fail-closed состояние для всех development environments и сохраняются при удалении environment stack. Единственным owner обоих guards является data-plane stack `primary`; stack другой environment не создаёт их custom resource, Lambda, IAM role либо `DataLakeSettings`, а до и после своего apply только проверяет фактическое account-global состояние. `primary` сохраняет exact существующие deployment principal и broad primary platform role в global administrator list, потому что их удаление потребовало бы запрещённой замены уже принятого account-global state. Другая development environment получает явные catalog, data-location, database и table permissions только на собственные ресурсы. Поэтому добавление environment не создаёт конкурирующего owner и не переписывает global authority либо authority другой environment.
