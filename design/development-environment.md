...
Trusted control source проверяет retained current link, оба release manifests, exact source/render digests и каждый tracked source byte до восстановления `/opt/workflow-infrastructure/current`. Любое выполнение Python непосредственно из infrastructure либо Product source release использует одновременно `-B` и `PYTHONDONTWRITEBYTECODE=1`, чтобы запрет переживал re-exec старого Product tool; host services и Helm post-renderer сохраняют тот же режим. Перед legacy Product `host-install` infrastructure атомарно устанавливает canonical WCC systemd drop-in для credential-refresh, а current WCC владеет тем же drop-in самостоятельно. Current source-manifest version явно фиксирует `python_bytecode_write_disabled=true`. Только recovery более старой версии manifest может удалить неманифестированные regular `*.pyc` непосредственно из regular `__pycache__`; current и будущие manifests не получают cleanup-исключение, а symlink, другой файл или вложенный каталог закрывает legacy cleanup. Cleanup сначала без mutation доказывает весь release и выполняется только если все расхождения исчерпываются разрешённым bytecode, после чего повторяется полное равенство фактического file graph манифесту. Ordinary activation ничего не очищает и принимает только уже точный graph.
