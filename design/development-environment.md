...
`destroy` является resumable idempotent operation exact task environment. Ни один stack или resource не обязан сохраняться до начала cleanup: уже отсутствующий exact target является успешным шагом. Перед каждым resource-owner phase и перед final proof resolver заново объединяет live stack outputs с deterministic bucket names, service-native exact-tag-filtered EC2/EBS reads и complete regional KMS key enumeration, отбирающей только exact `EnvironmentName` + `git-worktree`. Каждый output, discovered key и alias candidate непосредственно перечитывается через owning service; EC2/KMS identity, resource type, task tags, development account и region повторно доказываются до появления deletion authority и ещё раз непосредственно перед EC2/KMS mutation. Retained EBS owner непосредственно перед каждым snapshot или volume delete заново доказывает current account, region, resource identity/type, exact task tags и deletable state; stale resolver inventory не разрешает mutation. Каждый S3 read и mutation, поддерживающий `expected-bucket-owner`, передаёт exact development account, включая multipart/version lifecycle и final bucket deletion, поэтому один `HeadBucket` не становится переносимой authority. Journaled identifiers не участвуют в этом решении. Cleanup закрывает SSM sessions и stop leases, удаляет либо завершает все task instances и compute stack, а затем data stack до очистки retained buckets, чтобы сначала отозвать CloudFormation-owned IAM writers. После этого он удаляет все versions/delete markers/multipart uploads task buckets, все task retained volumes и one-time copy snapshots. Exact task KMS alias удаляется, все task keys отключаются и принимаются только после proof `PendingDeletion` с minimum AWS waiting period; физическое завершение service deletion не удерживает task cleanup. Account-foundation, primary, resources другого task prefix и untagged account-global guards не затрагиваются.

`destroy-inventory` выполняет fresh non-mutating enumeration и service-native readback всех owners и возвращает exact `external_resources_absent` вместе с current natural resource inventory. Successful `destroy` возвращает closed machine-readable result с exact request schema, common prefix и `external_resources_absent: true`. Private durable journal, keyed only by common prefix, хранит исключительно next phase progress и никогда не хранит resource inventory или deletion authority; каждый resume безопасно переигрывает все idempotent owners по fresh live state. Финальное доказательство начинается с отдельной fresh task-scoped enumeration, после которой CloudFormation доказывает отсутствие stacks, EC2 — accepted terminated/absent instances, volumes и snapshots, S3 — отсутствие buckets, KMS — отсутствие alias и accepted `PendingDeletion`/physical absence. Resource, ставший видимым после своего phase, блокирует success и попадает в owning phase следующего resume; polling отсутствует. Exact task tags являются discovery filter, а не blanket deletion selector. Foreign ownership и KMS state outside accepted transition закрывают truthful success; stale candidate после service-native absence не становится authority. Standard worktrees, branches and private task state удаляет `linear-agent-tools:task-cleanup` only after this result.
