...
Lake Formation account settings являются общей fail-closed guard, а не environment-local данными: они используют `Retain`, одинаковые defaults и только stable deployment principal в global administrator list. Platform role не добавляется в этот изменяемый account-global list; template явно выдаёт ей `CREATE_DATABASE`, `DATA_LOCATION_ACCESS`, database `ALL` и table-wildcard `ALL` в границах собственной environment. Поэтому runtime может создавать следующие Glue shards и назначать tenant query permissions, не снимая authority другой development environment при update.
