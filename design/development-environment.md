...
Root `worktree-bootstrap.yaml` schema v3 объявляет единственный typed handler key `workflow-infrastructure-development-environment`. Closed registry `linear-agent-tools:task-cleanup` связывает его с natural Project ID, owner issue, normalized repository identity и dated common prefix, сам выводит fixed `development_environment_manage.py destroy-inventory|destroy --git-worktree <common-prefix>` invocation и передаёт closed schema-v1 JSON request `{schema_version, common_prefix}` через stdin. Caller-supplied argv, approval fingerprint, cleanup receipt и parallel provider state отсутствуют. Product повторно доказывает non-primary environment identity и AWS ownership; завершение issue или Project само по себе handler не запускает.

`destroy` является restart-safe idempotent operation exact task environment, основанной только на current live state. Ни один stack или resource не обязан сохраняться до начала cleanup: уже отсутствующий exact target является успешным шагом. Перед каждым resource owner и перед final proof resolver заново объединяет live stack outputs с deterministic bucket names, service-native exact-tag-filtered EC2/EBS reads и complete regional KMS key enumeration, отбирающей только exact `EnvironmentName` + `git-worktree`. Каждый output, discovered key и alias candidate непосредственно перечитывается через owning service; EC2/KMS identity, resource type, task tags, development account и region повторно доказываются до появления deletion authority и ещё раз непосредственно перед mutation. Retained EBS owner непосредственно перед каждым snapshot или volume transition заново доказывает current account, region, resource identity/type, exact task tags и live state: `in-use` ждёт только `available`, `available` удаляется, `deleting` ждёт только `deleted`, а `deleted`/exact `NotFound` являются absence. Stale resolver inventory и progress files не разрешают mutation или restart. S3 owner проверяет deterministic inventory name, `expected-bucket-owner`, current bucket region и exact task tags непосредственно перед каждым abort/delete batch и final bucket deletion. CloudFormation owner заново читает exact stack name, parameters и task tags, выводит account/region-bound unique `StackId` ARN и использует только этот ARN для delete, waiter и absence readback; same-name replacement получает отдельную fresh ownership check и foreign replacement никогда не становится target. Cleanup сначала останавливает каждый discoverable running task instance, затем targeted-читает `describe-sessions --state Active`, завершает connected и disconnected sessions и свежим чтением доказывает их отсутствие; только после этого разрешены compute stack/instance deletion. Exact tagged EC2 inventory включает service-visible `terminated` tombstones, а exact current compute-stack output удерживает свой instance target даже после EC2 `NotFound` до pre-delete SSM proof, поэтому restart восстанавливает identity из live AWS owners, не полагаясь на run-local progress. После compute/data stack cleanup удаляются все versions/delete markers/multipart uploads task buckets, task retained volumes и one-time copy snapshots. Exact task KMS alias удаляется, все task keys отключаются и принимаются только после proof `PendingDeletion` с minimum AWS waiting period; физическое завершение service deletion не удерживает task cleanup. Account-foundation, primary, resources другого task prefix и untagged account-global guards не затрагиваются.

`destroy-inventory` выполняет fresh non-mutating enumeration и service-native readback всех owners и возвращает exact `external_resources_absent` вместе с current natural resource inventory. Successful `destroy` возвращает closed machine-readable result с exact request schema, common prefix и `external_resources_absent: true`. Cleanup journal и другая persisted progress authority отсутствуют: каждый restart безусловно повторяет все idempotent owners по fresh live state, поэтому stale, partial или unknown-format progress file игнорируется. После каждого полного owner pass выполняется отдельная fresh task-scoped enumeration и service-native absence readback; новый exact task resource запускает следующий полный pass в той же invocation до convergence, без sleep/time polling. Финальная проверка повторно требует CloudFormation stack absence, EC2 accepted terminated/absent state и session absence, volume/snapshot absence, S3 bucket absence, KMS alias absence и accepted `PendingDeletion`/physical absence. Exact task tags являются discovery filter, а не blanket deletion selector. Foreign ownership и KMS state outside accepted transition закрывают truthful success; stale candidate после service-native absence не становится authority. Standard worktrees, branches and private task state удаляет `linear-agent-tools:task-cleanup` only after this result.
