...
- установку exact k3s binary без внешнего install script;
...
До compute change set trusted operator resolver один раз выбирает latest stable release внутри объявленных линий `AWS CLI 2`, `Docker stable/noble`, `uv 0`, `Python 3.14`, `k3s 1.36` и `Helm 4`. Docker signing keyring обязан содержать ровно один primary key с закреплённым fingerprint; его subkeys допустимы, а второй primary trust anchor запрещён. `InRelease` проверяется через `gpgv`, а package digests берутся только из этого подписанного metadata graph. Exact uv tag/commit владеет Python download metadata; поэтому source сохраняет selector `3.14`, а canonical host-artifact manifest получает конкретные patch, build, shell-safe URL и SHA-256. Все выбранные bytes скачиваются и проверяются operator-side до change set. Canonical compressed manifest и digest являются единственными CloudFormation artifact inputs; отдельные дублирующие URL, version и digest parameters запрещены. UserData содержит manifest один раз, проверяет его digest, извлекает из него exact artifact graph и повторно проверяет каждый устанавливаемый byte. Размер manifest ограничен `3072` символами, а acceptance рендерит максимальный вход и доказывает соблюдение raw EC2 UserData limit `16 KiB`. Exact uv binary определяется digest; semantic version check сравнивает стабильные поля `uv <version>`, а информационный target-triple suffix standalone binary не является частью version identity.
...
Все Product lifecycle commands, включая `primary`, всегда передают exact
`--environment-name`. Отсутствующий selector и прежний CLI contract не
поддерживаются.
